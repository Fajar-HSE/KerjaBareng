...
function VerifyContent() {
  ...
  /* Token ada tapi belum diproses — sedang memverifikasi */
  ...
  const CONFIG = {
    success: {
      ...
      action:  <Link href="/login" className="btn btn-primary h-10 px-6 rounded-lg text-sm">Masuk Sekarang</Link>,
    },
    expired: {
      ...
      action:  <Link href="/register" className="btn btn-primary h-10 px-6 rounded-lg text-sm">Daftar Ulang</Link>,
    },
    invalid: {
    ...
    server: {
      ...
      action:  <Link href="/login" className="btn btn-secondary h-10 px-6 rounded-lg text-sm">Kembali ke Login</Link>,
    },
    pending: {
  ...
  /* Kalau masih ada token, trigger verify API */
